...
class Database{
	...
	public function query($q, $a = array()){
		...
		$this->cursor = mssql_query($this->conn, $q, $a);
		return $this->cursor;
	}
	...
	public function num_rows($cur = null){
		...
		if(!$cur) return false;
		$this->lastResult = mssql_num_rows($cur);
	...
	public function next_result($cur = null){
		if(!$cur) $cur = $this->cursor;
		if(!$cur) return false;
		$this->lastResult = mssql_next_result($cur);
		return $this->lastResult;
	}
	...
	public function fetch($cur = null){
		if(!$cur) $cur = $this->cursor;
		if(!$cur) return false;
		$this->lastResult = mssql_fetch($cur);
		return $this->lastResult;
	}
	...
	public function fetch_array($cur = null){
		if(!$cur) $cur = $this->cursor;
		if(!$cur) return false;
		$this->lastResult = mssql_fetch_array($cur);
		return $this->lastResult;
	}
	...
	public function fetch_object($cur = null){
		if(!$cur) $cur = $this->cursor;
		if(!$cur) return false;
		$this->lastResult = mssql_fetch_object($cur);
		return $this->lastResult;
	}
	...
	public function fetchAll($cur = null){
		...
		if(!$cur) return false;
		$all = array();
		while($this->lastResult = mssql_fetch_array($cur)){
			$all[] = $this->lastResult;
		}
		return $all;
	}
	...
	{
		$string = htmlentities(strip_tags(ms_escape_string($string)));
		...
		$search = array( // 4 per line
			...
			chr(150), chr(133), chr(194), chr(160), chr(255)
		);
	...
	public static function unclean($string)
	{
		...
		$string = preg_replace("/&ccedil;/", "ç", $string);
